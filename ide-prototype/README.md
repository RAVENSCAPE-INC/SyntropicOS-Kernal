...
- This is a minimal scaffold to iterate quickly. It uses the `preload` bridge to allow safe file I/O for the renderer.
 ...
 - The prototype records applied suggestions in `ide-prototype/suggestions.log` (JSON-lines). The Agent panel shows recent audit entries; each applied suggestion writes timestamp, file path, salience, backup path and the computed diff to the audit log.
 - The prototype records applied suggestions in `ide-prototype/suggestions.log` (JSON-lines). The Agent panel shows recent audit entries; each applied suggestion writes timestamp, file path, salience, backup path and the computed diff to the audit log. The agent-host can also return structured `edits` (line-based patches) that the IDE applies deterministically; the renderer will apply those edits when present instead of replacing the entire file.
